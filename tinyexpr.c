	...
	do {
		...
		if ((s->next[0] >= '0' && s->next[0] <= '9') || s->next[0] == '.') {
			...
			if (s->next[0] >= 'a' && s->next[0] <= 'z') {
				...
				case '>': s->type = TOK_INFIX; s->function = gt; break;
				case '<': s->type = TOK_INFIX; s->function = lt; break;
				case '&': s->type = TOK_INFIX; s->function = and; break;
				case '|': s->type = TOK_INFIX; s->function = or; break;
				case '=': s->type = TOK_INFIX; s->function = eq; break;
...
static te_expr *expr(state *s) {
	...
	while (s->type == TOK_INFIX && (s->function == and || s->function == or)) {
		te_fun2 t = s->function;
		next_token(s);
		ret = NEW_EXPR(TE_FUNCTION2 | TE_FLAG_PURE, ret, expr2(s));
		ret->function = t;
	}
